...
private:
  ...
  TABLE notestruct
  {
    ...
    std::string stakeAmount; // the note message
...
public:
  ...
  ACTION update(name user, std::string & stakeAmount)
  {
    ...
    if (isnewuser(user))
    {
      ...
      _notes.emplace(_self, [&](auto &new_user) {
        ...
        new_user.user = user;
        new_user.stakeAmount = stakeAmount;
    ...
    else
    {
      ...
      _notes.modify(note_entry, _self, [&](auto &modified_user) {
        modified_user.stakeAmount = stakeAmount;
